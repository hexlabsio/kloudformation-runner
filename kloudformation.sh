...
if [[ ! -f kloudformation.jar ]]; then
    if [[ ${QUITE} == "false" ]]; then echo Downloading KloudFormation 0.1.111; fi
    curl https://bintray.com/hexlabsio/kloudformation/download_file?file_path=io%2Fkloudformation%2Fkloudformation%2F0.1.111%2Fkloudformation-0.1.111-uber.jar -silent -L -o kloudformation.jar
fi
